USE bd_projeto;

INSERT INTO usuario(nome, email, senha, telefone, is_ativo, notificacao_vencimento, notificacao_manutencao, data_cadastro, possui_veiculo, tentativas_redefinir_senha)
VALUES ('Adriano', 'adriano@gmail.com', '$2a$10$.gUElnkNf25ckJfCnO8w6OVpdgK0tSAC2hZRwSPNLVkyMus3FGtHW', '00 00000-0000', TRUE, TRUE, TRUE, NOW(), TRUE, -1),
       ('Beto', 'beto@gmail.com', '$2a$10$.gUElnkNf25ckJfCnO8w6OVpdgK0tSAC2hZRwSPNLVkyMus3FGtHW', '00 00000-0001', TRUE, TRUE, TRUE, NOW(), TRUE, -1),
       ('Carla', 'carla@gmail.com', '$2a$10$.gUElnkNf25ckJfCnO8w6OVpdgK0tSAC2hZRwSPNLVkyMus3FGtHW', '00 00000-0002', TRUE, TRUE, TRUE, NOW(), TRUE, -1);

INSERT INTO veiculo(modelo, marca, ano, placa, cor, tipo, km_atual, id_usuario, is_ativo, data_ultima_atualizacao_km,intervalo_entre_manutencoes_km, intervalo_entre_manutencoes_meses, proxima_manutencao_Km, proxima_manutencao_data)
VALUES ('Onix Plus', 'Chevrolet', 2022, 'BTA2E19', 'Prata', 'CARRO', 45200, 1, TRUE, NOW(), 5000, 6, 50200, date_add(NOW(), INTERVAL 6 MONTH)),
       ('Cronos', 'Fiat', 2021, 'RLS3F42', 'Branco', 'CARRO', 78300, 2, TRUE, NOW(), 7500, 6, 85800, date_add(NOW(), INTERVAL 6 MONTH)),
       ('Corolla', 'Toyota', 2020, 'QNY4G11', 'Preto', 'CARRO', 112000, 3, TRUE, NOW(), 5000, 6, 117000, date_add(NOW(), INTERVAL 6 MONTH));

INSERT INTO meta(titulo, formato, valor_atual, valor_desejado, id_usuario, is_ativo, tipo)
VALUES ('Presente para a filha', 'SEMANAL', 0, 300.0, 1, TRUE, 'RECEITA'),
       ('Trocar bateria do celular', 'SEMANAL', 0, 200.0, 2, TRUE, 'CUSTO'),
       ('Jantar em restaurante japonês', 'SEMANAL', 0, 300.0, 3, TRUE, 'CUSTO');

INSERT INTO custo(tipo, valor, data_vencimento, data_pagamento, descricao, id_veiculo, is_ativo)
VALUES ('COMBUSTIVEL', 100.00, null, '2026-04-22', 'Abastecimento completo', 1, TRUE),
       ('MANUTENCAO', 200.0, null, '2026-03-20', 'Troca das pastilhas de freio dianteiras', 2, TRUE),
       ('MANUTENCAO', 150.0, null, '2026-03-25', 'Alinhamento e balanceamento', 3, TRUE);

INSERT INTO receita_diaria(data_receita, valor, id_usuario, is_ativo)
VALUES ('2026-04-21', 350.00, 1, TRUE),
       ('2026-04-02', 290.75, 2, TRUE),
       ('2026-04-02', 480.10, 3, TRUE);

INSERT INTO manutencao(tipo, data_manutencao, descricao, id_veiculo, id_custo, is_ativo)
VALUES ('CORRETIVA', '2026-03-20', 'Troca das pastilhas de freio dianteiras', 1, 2, TRUE),
       ('PREVENTIVA', '2026-03-25', 'Alinhamento e balanceamento', 2, 3, TRUE);
...
class ListarManutencaoServiceTest {
    ...
    @Test
    @DisplayName("Deve listar as manutenções paginadas corretamente")
    void deveListarManutencoesCorretamente() {
        ...
        when(manutencaoRepository.findAllByIsAtivo(true, pageable))
                .thenReturn(page);

        Page<Manutencao> response = tested.listar(pageable);

        verify(manutencaoRepository).findAllByIsAtivo(true, pageable);

        assertEquals(1, response.getTotalElements());
        assertEquals(manutencao.getIdManutencao(),
                response.getContent().get(0).getIdManutencao());
        assertEquals(manutencao.getDescricao(),
